...
int main(int argc, const char* argv[]) {
    ...
    const char* poem = "To be or not to be , that is the question ;\n"
...

    {
        ...
        */
        ...
        {
            cerr << "  common functions ";
            test(test_common(), true);
        }
        ...
        cerr << "  Number of classes: ";
        test(classdecoder.size(), 11); //including reserved classes
        ...
        cerr << "Ngram #1:" << endl;
        cerr << "Bytesize: ";
        test(ngram.bytesize(), 6);
        cerr << "Size (n): ";
        test(ngram.n(), 6);
        cerr << "Category==ngram: ";
        test(ngram.category(), NGRAM);
        cerr << "Hash: ";
        testgt(ngram.hash(), 0);
        cerr << "Raw: " << endl;
        ngram.out();
        cerr << "Decoded: ";
        test(ngram.decode(classdecoder), "To be or not to be");
        cerr << endl;
        ...
        cerr << "Bytesize: ";
        test(ngramhigh.bytesize(), 9);
        cerr << "Size (n): ";
        test(ngramhigh.n(), 3);
        cerr << "Category==ngram: ";
        test(ngramhigh.category(), NGRAM);
        cerr << "Hash: ";
        testgt(ngramhigh.hash(), 0);
        cerr << "Raw: " << endl;
        ngramhigh.out();
        cerr << "Decoded: ";
        test(ngramhigh.decode(classdecoder), "blah or blah");
        ...
        cerr << "Bytesize: ";
        test(ngramhigh2.bytesize(), 8);
        cerr << "Size (n): ";
        test(ngramhigh2.n(), 6);
        cerr << "Category==ngram: ";
        test(ngramhigh2.category(), NGRAM);
        cerr << "Hash: ";
        testgt(ngramhigh2.hash(), 0);
        cerr << "Raw: " << endl;
        ngramhigh2.out();
        cerr << "Decoded: ";
        test(ngramhigh2.decode(classdecoder), "to test or not to test");

        cerr << "Slice constructor ngram #1, specific subngram (low)" << endl;
        Pattern ngram2 = Pattern(ngram, 2, 2);

        cerr << "Ngram: ";
        test(ngram2.decode(classdecoder), "or not");
        cerr << "N: ";
        test(ngram2.n(), 2);
        ...
        cerr << "Ngram: ";
        test(ngramhighslice.decode(classdecoder), "or blah");
        cerr << "N: ";
        test(ngramhighslice.n(), 2);
        cerr << "Raw: " << endl;
        ngramhighslice.out();
        cerr << endl;
        ...
        cerr << "Ngram: ";
        test(ngramhighslice2.decode(classdecoder), "to test");
        cerr << "N: ";
        test(ngramhighslice2.n(), 2);
        cerr << "Bytesize: ";
        test(ngramhighslice2.bytesize(), 3);
        cerr << "Raw: " << endl;
        ngramhighslice2.out();
        cerr << endl;

        {
            ...
            cerr << "Decoding non-existing classes (should return {?}):";
            test(ngram.tostring(classdecoder2), "{?} {?} {?} {?} {?} {?}");
        }
        cerr << "----------------------------------------------------" << endl;
        cerr << "Copy constructor" << endl;
        Pattern ngramcopy(ngram);
        cerr << "Testing equality ";
        test(ngram == ngramcopy);
        cerr << endl;

        cerr << "Empty/null pattern" << endl;
        Pattern emptypattern;
        cerr << "N: ";
        test(emptypattern.n(), 0);
        cerr << "Bytesize: ";
        test(emptypattern.bytesize(), 0);
        ...
        for (const auto& subngram : tokens) {
            cerr << "#" << i << " -- ";
            test(subngram.decode(classdecoder), tokenref2[i]);
            i += 1;
        }
        cerr << "Count check ";
        test(i, 6);
        ...
        for (const auto& subngram : subngrams) {
            cerr << "#" << i << " -- ";
            test(subngram.decode(classdecoder), subngramref[i]);
            i += 1;
        }
        cerr << "Count check ";
        test(i, 21);
        ...
        cerr << "Testing occurrence of substring 'or not'? ";
        test(ngram.contains(subngram) == 1);

        string  substring2 = "to not";
        Pattern subngram2  = encoder.buildpattern(substring2, true);
        cerr << "Testing non-occurrence of substring 'to not'? ";
        test(ngram.contains(subngram2) == 0);

        string  substring3 = "to be";
        Pattern subngram3  = encoder.buildpattern(substring3, true);
        cerr << "Testing occurrence of substring 'to be'? ";
        test(ngram.contains(subngram3) == 1);

        string  substring4 = "or";
        Pattern subngram4  = encoder.buildpattern(substring4, true);
        cerr << "Testing occurrence of substring 'or'? ";
        test(ngram.contains(subngram4) == 1);
        ...
        cerr << "Reverse ngram: ";
        test(revngram.decode(classdecoder), "be to not or be To");
        cerr << "N: ";
        test(revngram.n(), 6);
        ...
        cerr << "Reverse ngram: ";
        test(revhingram.decode(classdecoder), "not or blah blah to");
        cerr << "N: ";
        test(revhingram.n(), 5);
        ...
        cerr << "Testing equivalence between pointer and pattern";
        test(ngram == pngram);
        cerr << "Testing equivalence between pointer and pattern (rev)";
        test(pngram == ngram);
        cerr << "Testing hash equivalence between pointer and pattern";
        test(ngram.hash() == pngram.hash());
        Pattern derefngram(pngram);
        cerr << "Testing equivalence after pointer construction and dereference";
        test(ngram == derefngram);
        cerr << "Testing equivalence after pointer construction and dereference (rev)";
        test(derefngram == ngram);
        cerr << "Testing equivalence between pointer from pattern and pointer";
        test(PatternPointer(ngram) == pngram);
        cerr << "Testing equivalence between pointer from pattern and pointer (rev)";
        test(pngram == PatternPointer(ngram));

        cerr << "Bytesize: ";
        test(pngram.bytesize(), 6);
        cerr << "Size (n): ";
        test(pngram.n(), 6);
        cerr << "Category==ngram: ";
        test(pngram.category(), NGRAM);
        cerr << "Raw: " << endl;
        ngram.out();
        cerr << "Decoded: ";
        test(pngram.decode(classdecoder), "To be or not to be");

        cerr << "Slice constructor, specific subngram (low only)" << endl;
        PatternPointer pngramslice = PatternPointer(pngram, 2, 2);
        cerr << "Ngram: ";
        test(pngramslice.decode(classdecoder), "or not");
        cerr << "N: ";
        test(pngramslice.n(), 2);
        ...
        cerr << "Bytesize: ";
        test(pngramhigh.bytesize(), 9);
        cerr << "Size (n): ";
        test(pngramhigh.n(), 3);
        cerr << "Category==ngram: ";
        test(pngramhigh.category(), NGRAM);
        cerr << "Hash: ";
        testgt(pngramhigh.hash(), 0);
        cerr << "Raw: " << endl;
        pngramhigh.out();
        cerr << "Decoded: ";
        test(pngramhigh.decode(classdecoder), "blah or blah");

        cerr << "Slice constructor, specific subngram (high)" << endl;
        Pattern pngramhighslice = PatternPointer(pngramhigh, 1, 2);
        cerr << "Ngram: ";
        test(pngramhighslice.decode(classdecoder), "or blah");
        cerr << "N: ";
        test(pngramhighslice.n(), 2);
        cerr << "Raw: " << endl;
        pngramhighslice.out();
        cerr << endl;
        ...
        cerr << "Bytesize: ";
        test(pngramhigh2.bytesize(), 8);
        cerr << "Size (n): ";
        test(pngramhigh2.n(), 6);
        cerr << "Category==ngram: ";
        test(pngramhigh2.category(), NGRAM);
        cerr << "Hash: ";
        testgt(pngramhigh2.hash(), 0);
        cerr << "Raw: " << endl;
        pngramhigh2.out();
        cerr << "Decoded: ";
        test(pngramhigh2.decode(classdecoder), "to test or not to test");

        cerr << "Slice constructor, specific subngram (high 2)" << endl;
        PatternPointer pngramhighslice2 = PatternPointer(pngramhigh2, 0, 2);
        cerr << "Ngram: ";
        test(pngramhighslice2.decode(classdecoder), "to test");
        cerr << "N: ";
        test(pngramhighslice2.n(), 2);
        cerr << "Bytesize: ";
        test(pngramhighslice2.bytesize(), 3);
        cerr << "Raw: " << endl;
        ...
        cerr << "Ngram: ";
        test(ngramhighslice3.decode(classdecoder), "to test");
        cerr << "N: ";
        test(ngramhighslice3.n(), 2);
        cerr << "Bytesize: ";
        test(ngramhighslice3.bytesize(), 3);
        cerr << "Raw: " << endl;
        ngramhighslice3.out();
        cerr << endl;
        ...
        for (const auto& subngram1 : ptokens) {
            cerr << "#" << i << " -- ";
            test(subngram1.decode(classdecoder), tokenref[i]);
            i += 1;
        }
        cerr << "Count check ";
        test(i, 6);
        ...
        for (const auto& psubngram : psubngrams) {
            cerr << "#" << i << " -- ";
            test(psubngram.decode(classdecoder), subngramref[i]);
            i += 1;
        }
        cerr << "Count check ";
        test(i, 21);
        ...
        for (const auto& psubngram2 : psubngrams2) {
            cerr << "#" << i << " -- ";
            test(psubngram2.decode(classdecoder), subngramref[i]);
            i += 1;
        }
        cerr << "Count check ";
        test(i, 21);
        ...
        cerr << "Ngram: ";
        test(ngramconc.decode(classdecoder), "To be or not to be");
        cerr << "N: ";
        test(ngramconc.n(), 6);
        cerr << "Bytesize: ";
        test(ngramconc.bytesize());
        cerr << "Raw: " << endl;
        ngramconc.out();
        cerr << "----------------------------------------------------" << endl;
        emptypattern = emptypattern + ngramconc;
        cerr << "Adding to empty? ";
        test(emptypattern == ngramconc);
        cerr << "----------------------------------------------------" << endl;
        cerr << "Ngram comparison: " << endl;
        cerr << "Equality? ";
        test(ngramconc == ngram);
        cerr << "Non-equality? ";
        test(ngramconc != ngrambegin);
        cerr << "greater than? ";
        test(subngram2 > subngram3);
        cerr << "greater than? ";
        test(subngram4 > subngram3);
        cerr << "less than? ";
        test(subngram3 < subngram2);
        cerr << "less than? ";
        test(subngram5 < subngram3);
        cerr << "Begin is less? ";
        test(ngrambegin < ngramconc);
        cerr << "equivalence under less? ";
        test(!(subngram < subngram));
        ...
        } catch (const UnknownTokenError& e) { caught = true; }
        test(caught, true);

        Pattern unkgram2 = encoder.buildpattern(unkstring, true);
        cerr << "Encode as unknown when allowunknown=true";
        test(unkgram2.decode(classdecoder), "to {?} {?} or not");
        ...
        cerr << "Skipgram decoded: ";
        test(skipgram.decode(classdecoder), "To {*} or {*} to be");
        cerr << "N: ";
        test(skipgram.n(), 6);
        cerr << "Bytesize: ";
        test(skipgram.bytesize(), 6);
        cerr << "Category==skipgram? ";
        test(skipgram.category() == SKIPGRAM);
        cerr << "Skipcount==2? ";
        test(skipgram.skipcount(), 2);
        ...
        for (const auto& part : parts) {
            cerr << "#" << i << " -- ";
            test(part.decode(classdecoder), partsref[i]);
            i += 1;
        }
        cerr << "Count check ";
        test(i, 3);
        ...
        i                        = 0;
        for (const auto& [i1, i2] : gapcontainer) {
            cerr << "#" << i << " -- begin ";
            test(i1, gaprefbegin[i]);
            cerr << "#" << i << " -- length ";
            test(i2, gapreflength[i]);
            i += 1;
        }
        cerr << "Count check ";
        test(i, 2);
        cerr << "Instanceof test (positive) ";
        test(ngram.instanceof (skipgram));

        string  querystring2_1gap = "To be or {*1*} to be";
        Pattern skipgram_1gap     = encoder.buildpattern(querystring2_1gap, true);
        cerr << "Instanceof test with less abstract skipgram ";
        test(skipgram_1gap.instanceof (skipgram));
        cerr << "Instanceof test (ngram equality) ";
        test(ngram.instanceof (ngram));

        Pattern revskipgram = skipgram.reverse();
        cerr << "Reverse skipgram: ";
        test(revskipgram.decode(classdecoder), "be to {*} or {*} To");
        cerr << "N: ";
        test(revskipgram.n(), 6);
        cerr << "Instanceof test (negative) ";
        test(!ngram.instanceof (revskipgram));
        ...
        cerr << "Skipgram decoded: ";
        test(pskipgram.decode(classdecoder), "To {*} or {*} to be");
        cerr << "N: ";
        test(pskipgram.n(), 6);
        cerr << "Bytesize: ";
        test(pskipgram.bytesize(), 6);
        cerr << "Category==skipgram? ";
        test(pskipgram.category() == SKIPGRAM);
        cerr << "Skipcount==2? ";
        test(pskipgram.skipcount(), 2);
        cerr << "Testing equivalence between pointer and different-source pointer";
        test(pskipgram == pskipgram2);
        cerr << "Testing equivalence between pointer and pattern";
        test(skipgram == pskipgram);
        cerr << "Testing equivalence between pointer and pattern (rev)";
        test(pskipgram == skipgram);
        cerr << "Testing hash equivalence between pointer and pattern";
        test(skipgram.hash() == pskipgram.hash());
        Pattern derefskipgram = Pattern(pskipgram);
        cerr << "Testing equivalence after pointer construction and dereference";
        test(skipgram == derefskipgram);
        cerr << "Testing equivalence after pointer construction and dereference (rev)";
        test(derefskipgram == skipgram);
        cerr << "Testing equivalence between pointer from pattern and pointer";
        test(PatternPointer(skipgram) == pskipgram);
        cerr << "Testing equivalence between pointer from pattern and pointer (rev)";
        test(pskipgram == PatternPointer(skipgram));
        ...
        test(i, 2);
        cerr << "Instanceof test (positive) ";
        test(ngram.instanceof (pskipgram));
        ...
        cerr << "Skipcontent: ";
        test(skipcontent.decode(classdecoder), "be {*} not");
        cerr << "N: ";
        test(skipcontent.n(), 3);
        cerr << "Bytesize: ";
        test(skipcontent.bytesize(), 3);
    ...
    */
        ...
        cerr << "Skipgram: ";
        test(skipgraminv2.decode(classdecoder), "be {*} not");
        cerr << "N: ";
        test(skipgraminv2.n(), 3);
        cerr << "Bytesize: ";
        test(skipgraminv2.bytesize(), 3);
        ...
        for (const auto& part : parts2) {
            cerr << "#" << i << " -- ";
            test(part.decode(classdecoder), partsref2[i]);
            i += 1;
        }
        cerr << "Count check ";
        test(i, 2);
        ...
        cerr << "Skipgram: ";
        test(skipgram5.decode(classdecoder), "be {*} not {*} {*} be");
        ...
        for (const auto& part : parts2) {
            cerr << "#" << i << " -- ";
            test(part.decode(classdecoder), skipgram5partsref[i]);
            i += 1;
        }
        cerr << "Count check ";
        test(i, 3);
        ...
        for (const auto& [i1, i2] : gaps) {
            cerr << "#" << i << " -- begin ";
            test(i1, skipgram5gaprefbegin[i]);
            cerr << "#" << i << " -- length ";
            test(i2, skipgram5gapreflength[i]);
            i += 1;
        }
        cerr << "Count check ";
        test(i, 2);
        ...
        for (const auto& [i1, i2] : p) {
            cerr << "#" << i << " -- begin ";
            test(i1, skipgram5partrefbegin[i]);
            cerr << "#" << i << " -- length ";
            test(i2, skipgram5partreflength[i]);
            i++;
        }
        cerr << "Count check ";
        test(i, 3);
        ...
        for (const auto& part : parts2) {
            cerr << "#" << i << " -- ";
            test(part.decode(classdecoder), skipgram7partsref[i]);
            i += 1;
        }
        cerr << "Count check ";
        test(i, 3);
        ...
        for (const auto& [g1, g2] : gaps) {
            cerr << "#" << i << " -- begin ";
            test(g1, skipgram7gaprefbegin[i]);
            cerr << "#" << i << " -- length ";
            test(g2, skipgram7gapreflength[i]);
            i++;
        }
        cerr << "Count check ";
        test(i, 2);
        ...
        for (const auto& part : parts2) {
            cerr << "#" << i << " -- ";
            test(part.decode(classdecoder), skipgram8partsref[i]);
            i++;
        }
        cerr << "Count check ";
        test(i, 1);
        ...
        for (const auto& [g1, g2] : gaps) {
            cerr << "#" << i << " -- begin ";
            test(g1, skipgram8gaprefbegin[i]);
            cerr << "#" << i << " -- length ";
            test(g2, skipgram8gapreflength[i]);
            i++;
        }
        cerr << "Count check ";
        test(i, 2);
        ...
        cerr << "Size (n): ";
        test(flexgram5.n(), 5); //== size()
        cerr << "Bytesize: ";
        test(flexgram5.bytesize(), 5);
        cerr << "Category==flexgram: ";
        test(flexgram5.category() == FLEXGRAM);
        cerr << "Raw" << endl;
        flexgram5.out();
        cerr << "Parts: " << endl;
        ...
        for (const auto& part : flexparts) {
            cerr << "#" << i << " -- ";
            test(part.decode(classdecoder), flexrefparts[i]);
            i++;
        }
        cerr << "Count check ";
        test(i, 3);
        ...
        in.close();
        cerr << "Integrity check for ngram? ";
        test(ngram == ngram_read);
        cerr << "Integrity check for ngram (high)? ";
        test(ngramhigh == ngramhigh_read);
        cerr << "Integrity check for empty pattern? ";
        test(empty == empty_read);
        cerr << "Integrity check for skipgram? ";
        test(skipgram == skipgram_read);
        ...
        map1[empty]     = 1;
        map1[flexgram5] = 2;
        cerr << "Integrity for PatternMap";
        test(map1.size(), 4);
        cerr << "Integrity for PatternMap";
        test(map1[ngram], 1);
        cerr << "Integrity for PatternMap";
        test(map1[ngramhigh], 1);
        cerr << "Integrity for PatternMap";
        test(map1[empty], 1);
        cerr << "Integrity for PatternMap";
        test(map1[flexgram5], 2);
        ...
        test(map2[ngram], 1);
        cerr << "Integrity for PatternMap";
        test(map2[ngramhigh], 1);
        cerr << "Integrity for PatternMap";
        test(map2[empty], 1);
        cerr << "Integrity for PatternMap";
        test(map2[flexgram5], 2);

        AlignedPatternMap<uint32_t> amap1;
        amap1[ngram][ngram]         = 1;
        amap1[flexgram5][flexgram5] = 2;
        cerr << "Integrity for AlignedPatternMap";
        test(amap1.size(), 2);
        cerr << "Integrity for AlignedPatternMap";
        test(amap1[ngram][ngram], 1);
        cerr << "Integrity for AlignedPatternMap";
        test(amap1[flexgram5][flexgram5], 2);
        ...
        test(amap2.size(), 2);
        cerr << "Integrity for AlignedPatternMap";
        test(amap2[ngram][ngram], 1);
        cerr << "Integrity for AlignedPatternMap";
        test(amap2[flexgram5][flexgram5], 2);

        PatternPointerMap<uint32_t> ppmap;
        ppmap[pngram]    = 1;
        ppmap[pskipgram] = 3;
        cerr << "Integrity for PatternPointerMap";
        test(ppmap.size(), 2);
        cerr << "Integrity for PatternPointerMap";
        test(ppmap[pngram], 1);
        cerr << "Integrity for PatternPointerMap";
        test(ppmap[pskipgram], 3);
        cerr << "Querying PatternPointerMap with Pattern (ngram)";
        test(ppmap[ngram], 1);
        cerr << "Querying PatternPointerMap with Pattern (skipgram)";
        test(ppmap[skipgram], 3);
        cerr << "Querying PatternPointerMap for non-existing pattern";
        test(ppmap[flexgram5], 0);
...

    {
        ...
        cerr << "Computing possible gaps in 3-grams: ";
        test(masks.size(), 1);

        masks = compute_skip_configurations(4, 4);
        cerr << "Computing possible gaps in 4-grams: ";
        test(masks.size(), 3);
        ...
        for (const auto& pmask : masks) {
            ...
            cerr << "   pattern vs patternpointer equivalence: ";
            test(skipgram == pskipgram);
            cerr << "   patternpointer vs patternpointer equivalence: ";
            test(pskipgram == pskipgram2);
            cerr << "   pattern from patternpointer equivalence: ";
            test(Pattern(pskipgram) == skipgram);
        }
        cerr << "Count check: ";
        test(masks.size(), 15);
    }

    {
        ...
        *out << string(poem);
        ...
        cerr << "Checking equivalence after decoding: ";
        test(res, 0);
        ...
        cerr << "First word:  ";
        test(firstword.tostring(classdecoder), "To");
        ...
        cerr << "Patterns";
        test(unindexedmodelNSR.size(), 111);
        cerr << "Types";
        test(unindexedmodelNSR.types(), 186);
        cerr << "Tokens";
        test(unindexedmodelNSR.tokens(), 354);

        cerr << "Unigram Types";
        test(unindexedmodelNSR.totalwordtypesingroup(0, 1), 45);
        ...
        cerr << "Patterns";
        test(unindexedmodelNS.size(), 111);
        cerr << "Types";
        test(unindexedmodelNS.types(), 186);
        cerr << "Tokens";
        test(unindexedmodelNS.tokens(), 354);
        ...
        cerr << "Testing unindexedmodel.has()";
        test(unindexedmodelNS.has(ngramNS));
        Pattern ngramNS_ne = classencoder.buildpattern("give us fortune", true);
        cerr << "Testing !unindexedmodel.has()";
        test(!unindexedmodelNS.has(ngramNS_ne));
        cerr << "Testing unindexedmodel.occurencecount()";
        test(unindexedmodelNS.occurrencecount(ngramNS), 6);
        ...
        cerr << "Patterns";
        test(unindexedmodelR.size(), 385);
        cerr << "Types";
        test(unindexedmodelR.types(), 186);
        cerr << "Tokens";
        test(unindexedmodelR.tokens(), 354);
        ...
        cerr << "Patterns";
        test(unindexedmodel.size(), 385);
        cerr << "Types";
        test(unindexedmodel.types(), 186);
        cerr << "Tokens";
        test(unindexedmodel.tokens(), 354);
        ...
        cerr << "Testing unindexedmodel.has()";
        test(unindexedmodel.has(ngram));
        Pattern ngram_ne = classencoder.buildpattern("give us fortune", true);
        cerr << "Testing !unindexedmodel.has()";
        test(!unindexedmodel.has(ngram_ne));
        cerr << "Testing unindexedmodel.occurencecount()";
        test(unindexedmodel.occurrencecount(ngram), 6);
        ...
        cerr << "Equal tokens? ";
        test(unindexedmodel.tokens() == unindexedmodel2.tokens());
        cerr << "Equal types? ";
        test(unindexedmodel.types() == unindexedmodel2.types());
        cerr << "Equal size? ";
        test(unindexedmodel.size() == unindexedmodel2.size());
        cerr << "Testing unindexedmodel2.has()";
        test(unindexedmodel2.has(ngram));
        cerr << "Testing unindexedmodel2.occurrencecount()";
        test(unindexedmodel2.occurrencecount(ngram), 6);
        ...
        cerr << "Size test";
        test(indexedmodel.size(), 133);
        cerr << "Size test (2) ";
        test(indexedmodel.size(), unindexedmodelNS.size() + indexedmodel.totalpatternsingroup(SKIPGRAM, 0));
        cerr << "Equal tokens? ";
        test(unindexedmodel.tokens() == indexedmodel.tokens());
        cerr << "Equal types? ";
        test(unindexedmodel.types() == indexedmodel.types());
        cerr << "Testing indexedmodel.has()";
        test(indexedmodel.has(ngram));
        cerr << "Testing indexedmodel.occurrencecount()";
        test(indexedmodel.occurrencecount(ngram), 6);
        ...
        cerr << "Unigram Types";
        test(indexedmodel.totalwordtypesingroup(0, 1), 45);
        ...
        options.DEBUG                       = false;
        cerr << "Equal size? ";
        test(indexedmodel2.size(), indexedmodel.size());
        cerr << "Equal tokens? ";
        test(indexedmodel2.tokens(), indexedmodel.tokens());
        cerr << "Equal types? ";
        test(indexedmodel2.types(), indexedmodel2.types());
        ...
        cerr << "Equal size? ";
        test(indexedasunindexedmodel.size(), indexedmodel.size());
        cerr << "Equal tokens? ";
        test(indexedasunindexedmodel.tokens(), indexedmodel.tokens());
        cerr << "Equal types? ";
        test(indexedasunindexedmodel.types(), indexedmodel2.types());
        ...
        for (const auto& [rel, dummy] : relations6) {
            ...
            test(rel.n(), 3);
            cerr << " type check: ";
            test(rel.category(), NGRAM);
        }
        cerr << "Verify count: ";
        test(relations6.size(), 2); //the other 2 are below occurrence threshold
        ...
        cerr << "Verify count: ";
        test(relations7.size(), 2); //other two are below threshold
        ...
        for (const auto& [rel, dummy] : relations8) {
            ...
            test(rel.n(), 7);
            cerr << " type check: ";
            test(rel.category(), SKIPGRAM); //no flexgrams in model at this point
        }
        cerr << "Verify count: ";
        test(relations8.size(), 8); //other two are below threshold
        ...
        cerr << "Verify count: ";
        test(relations62.size(), 2); //other two are below threshold
        ...
        for (const auto& iter : relations72) {
            ...
            test(iter.first.n(), 7);
            cerr << " type check: ";
            test(iter.first.category(), NGRAM);
        }
        cerr << "Verify count: ";
        test(relations72.size(), 2); //other two are below threshold
        ...
        if (verbose) {
            ...
            for (const auto& iter : indexedmodel) {
                ...
                for (const auto& iter2 : indexedmodel) {
                    ...
                    cerr << "\t\t...p vs p ";
                    test(pattern == pattern2, ii == jj);
                    cerr << "\t\t...pp vs pp ";
                    test(pp == pp2, ii == jj);
                    cerr << "\t\t...p vs pp ";
                    test(pattern == pp2, ii == jj);
                    cerr << "\t\t...pp vs p ";
                    test(pp == pattern2, ii == jj);
                    ++jj;
                }
                ++ii;
            }
        ...
        cerr << "Patterns";
        test(unindexedmodel3.size(), 385);
        cerr << "Types";
        test(unindexedmodel3.types(), 186);
        cerr << "Tokens";
        test(unindexedmodel3.tokens(), 354);
    ...
    }
    {
        ...
        for (const auto& [ref, pp] : matches) {
            ...
            cerr << "   testing match equivalence: ";
            test(pp == skipgram);
            cerr << "   testing reference: ";
            test(ref == IndexReference(1, 7));
            ++ii;
        }
        cerr << "Size check: ";
        test(1, ii);
    ...
    }
    {
        ...
        cerr << "Count check ";
        test(ui, ppmodel.size());

        cerr << "Checking presence of Pattern";
        test(ppmodel[ngram], 7);
        cerr << "Checking presence of PatternPointer";
        test(ppmodel[pngram], 7);
        cerr << "Querying occurrencecount with Pattern (ngram)";
        test(ppmodel.occurrencecount(ngram), 7);
        cerr << "Querying occurrencecount with PatternPointer (ngram)";
        test(ppmodel.occurrencecount(pngram), 7);
        string  querystring2 = "see or not to see";
        Pattern ngram2       = classencoder.buildpattern(querystring2);
        cerr << "Querying occurrencecount with Pattern (ngram) (2)";
        test(ppmodel.occurrencecount(ngram2), 2);
        string         querystring = "or not to {*} .";
        Pattern        skipgram    = classencoder.buildpattern(querystring, false);
        PatternPointer pskipgram   = PatternPointer(skipgram);
        cerr << "Querying occurrencecount with PatternPointer (skipgram)";
        test(ppmodel.occurrencecount(pskipgram), 4);
        cerr << "Querying occurrencecount with Pattern (skipgram)";
        test(ppmodel.occurrencecount(skipgram), 4);
        ...
        cerr << "Verifying equal size";
        test(ppmodel.size(), refmodel.size());
        cerr << "Verifying equal token count";
        test(ppmodel.tokens(), refmodel.tokens());
        cerr << "Verifying equal type count";
        test(ppmodel.types(), refmodel.types());
        ...
        cerr << "Verifying equal size";
        test(ppmodel2.size(), ppmodel.size());
        cerr << "Verifying equal token count";
        test(ppmodel2.tokens(), ppmodel.tokens());
        cerr << "Verifying equal type count";
        test(ppmodel2.types(), ppmodel.types());
        ...
        cerr << "Verifying equal size";
        test(ppmodel3.size(), ppmodel.size());
        cerr << "Verifying equal token count";
        test(ppmodel3.tokens(), ppmodel.tokens());
        cerr << "Verifying equal type count";
        test(ppmodel3.types(), ppmodel.types());
    }
    {
        ...
        cerr << "Checking presence of Pattern";
        test(ppmodel[ngram].count(), 7);
        cerr << "Checking presence of PatternPointer";
        test(ppmodel[pngram].count(), 7);
        cerr << "Querying occurrencecount with Pattern (ngram)";
        test(ppmodel.occurrencecount(ngram), 7);
        cerr << "Querying occurrencecount with PatternPointer (ngram)";
        test(ppmodel.occurrencecount(pngram), 7);
    ...
    }
    {
        ...
        cerr << "Checking set size: ";
        test(patternset.size(), 3);

        std::set<Pattern>::iterator iter = patternset.begin();
        cerr << "Set integrity #1: ";
        test(iter->tostring(decoder), querystring3);
        ++iter;
        cerr << "Set integrity #2: ";
        test(iter->tostring(decoder), querystring2);
        ++iter;
        cerr << "Set integrity #3: ";
        test(iter->tostring(decoder), querystring);
    }
    return EXIT_SUCCESS;
}
