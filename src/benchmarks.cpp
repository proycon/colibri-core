...
int main(int argc, const char* argv[]) {
    ...
    if (testnr == 101) {
        Measurement m =
            begin(string("101 - Training indexed PatternModel from preloaded corpus: threshold 2, up to 8-grams, with skipgrams, skip-type threshold 12, occurence 25"));
        PatternModelOptions options;
        options.MINTOKENS           = 2;
        options.MINTOKENS_SKIPGRAMS = 25, options.MAXLENGTH = 8;
        options.DOSKIPGRAMS  = true;
        options.MINSKIPTYPES = 12;
        IndexedPatternModel<> model(&corpus);
        model.train(datafile, options);
    ...
    if (testnr == 102) {
        Measurement m =
            begin(string("102 - Training indexed PatternModel Pointer from preloaded corpus: threshold 2, up to 8-grams, with skipgrams, skip-type threshold 12, occurence 25"));
        PatternModelOptions options;
        options.MINTOKENS           = 2;
        options.MINTOKENS_SKIPGRAMS = 25, options.MAXLENGTH = 8;
        options.DOSKIPGRAMS  = true;
    ...
    if (testnr == 103) {
        ...
        PatternModelOptions options;
        options.MINTOKENS              = 10;
        options.MAXLENGTH              = 4;
        options.MINTOKENS_SKIPGRAMS    = 20;
        options.DOSKIPGRAMS_EXHAUSTIVE = true;
        PatternPointerModel<uint32_t> model(&corpus);
        model.train(datafile, options);
        end(m);
