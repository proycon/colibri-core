...
bool PatternPointer::operator==(const Pattern& other) const {
    ...
    while (i < bytes) {
        if ((i > 0) && (other.data[i - 1] >= 128) && (other.data[i] == 0))
            return false;
        if ((mask != 0) && (data[i] < 128)) {
            if (isgap(_n)) {
                if (other.data[i] != ClassDecoder::skipclass)
                    return false;
...
bool PatternPointer::operator==(const PatternPointer& other) const {
    ...
    if (bytes == other.bytes) {
        if ((mask != 0) && (isflexgram())) {
            if ((other.mask == 0) || (!other.isflexgram()))
                return false;
...
int PatternPointer::parts(vector<pair<int, int>>& container) const {
    ...
    for (size_t i = 0; (i < bytes) && (i < 31); i++) {
        ...
        if (c < 128) {
            //low byte, end of token
            if ((mask & bitmask[_n]) == 0) {
                partlength = _n - partbegin;
                if (partlength > 0) {
                    container.push_back(pair<int, int>(partbegin, partlength));
                    found++;
                }
    ...
    do {
        ...
        if ((!prevhigh) && (c == ClassDecoder::delimiterclass)) {
            ...
            return count;
                   ...
                   (((c == ClassDecoder::skipclass) || (c == ClassDecoder::flexclass)) &&
                    ((i > 0) || ((data[i - 1] != ClassDecoder::skipclass) && (data[i - 1] != ClassDecoder::flexclass))))) {
            //we have a marker
            count++;
            i++;
        } else {
    ...
    do {
        if (data[i] < 128) {
            if (isgap(_n)) {
                ...
                if (beginskip > -1)
                    container.push_back(pair<int, int>(beginskip, skiplength));
                beginskip = -1;
            }
            _n++;
        }
...
int Pattern::gaps(vector<pair<int, int>>& container) const {
    ...
    int endskip = 0;
    for (int i = bs; i > 0; i--) {
        if ((data[i - 1] >= 128) && ((data[i] == ClassDecoder::skipclass) || (data[i] == ClassDecoder::flexclass))) {
            endskip++;
        } else {
            break;
        }
    ...
    if (endskip)
        container.push_back(pair<int, int>(_n - endskip, endskip));
    return container.size();
}
...
bool Pattern:: instanceof (const PatternPointer& skipgram) const {
    ...
    if (skipgram.category() == FLEXGRAM) {
        ...
        if (flex_n < _n)
            return false; //too small too match
        ...
        for (size_t i = 0; i < _n; i++) {
            ...
            if ((reftoken != token) && (reftoken.category() != SKIPGRAM))
                return false;
        }
        return true;
    }
...
bool PatternPointer:: instanceof (const PatternPointer& skipgram) const {
    ...
    if (skipgram.category() == FLEXGRAM) {
        ...
        for (size_t i = 0; i < _n; i++) {
            const PatternPointer reftoken = PatternPointer(skipgram, i, 1);
            const PatternPointer token    = PatternPointer(*this, i, 1);
            if ((token != reftoken) && (reftoken.category() != SKIPGRAM))
                return false;
        }
        return true;
    }
...
Pattern Pattern::addskip(const std::pair<int, int>& gap) const {
    ...
    if (pattern.n() != _n) {
        std::cerr << "ERROR: addskip(): Pattern length changed from " << _n << " to " << pattern.n() << " after substituting slice (" << gap.first << "," << gap.second << ")"
                  << std::endl;
        throw InternalError();
    }
    return pattern;
...
Pattern Pattern::addskips(const std::vector<std::pair<int, int>>& gaps) const {
    ...
    for (vector<pair<int, int>>::const_iterator iter = gaps.begin(); iter != gaps.end(); ++iter) {
        ...
        if (pattern.n() != _n) {
            std::cerr << "ERROR: addskip(): Pattern length changed from " << _n << " to " << pattern.n() << " after substituting slice (" << iter->first << "," << iter->second
                      << ")" << std::endl;
            throw InternalError();
        }
    }
...
void IndexedCorpus::load(std::istream& in, bool debug) {
    ...
    if (version == 2) {
        ...
        if (debug)
            std::cerr << "Reading " << corpussize << " bytes" << std::endl;
    ...
    if (sentence - 1 != sentenceindex.size()) {
        std::cerr << "ERROR: IndexedCorpus expected " << sentence - 1 << " sentence, but loaded only " << sentenceindex.size() << "!! Max size is " << sentenceindex.max_size()
                  << std::endl;
        throw InternalError();
    }
    if (debug)
        cerr << "Loaded " << sentenceindex.size() << " sentences; corpussize (bytes) = " << corpussize << endl;
...
PatternPointer IndexedCorpus::findpattern(const IndexReference& begin, const Pattern& pattern, PatternCategory resultcategory) const {
    if (pattern.category() == NGRAM) {
        ...
        if (resultcategory == NGRAM)
            return result;
        result.mask = pattern.getmask(); //to skipgram
        if (resultcategory == FLEXGRAM)
            result.mask = result.mask | (unsigned(1) << 31); //make flexgram
...
void IndexedCorpus::findpattern(std::vector<std::pair<IndexReference, PatternPointer>>& result, const Pattern& pattern, uint32_t sentence, bool instantiate) {
    ...
    int       sentence_length = this->sentencelength(sentence);
    for (int i = 0; i < sentence_length - _n; i++) {
