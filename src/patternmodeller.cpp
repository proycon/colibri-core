...
void usage() {
    ...
    cerr << "\t-j|--constraintmodel [modfile]  Joined input model, i.e. constraint model/training model. Result will be the *intersection* of this (training) model and the input "
            "model or constructed model. For training, consider using -I instead."
         << endl;
    cerr << endl;
    cerr << " Building a model:  colibri-patternmodeller -o [modelfile] -f [datafile] -c [classfile]" << endl;
    cerr << "\t-2|--twostage                 Enable two-stage building (for indexed models), takes longer but saves a lot of memory on large corpora! First builds an unindexed "
            "model and reuses that (via -I) to"
         << endl;
    ...
    cerr << "\t-b|--backofflength <number>   Maximum back-off length (default: 100). Only makes sense to set lower than minimum pattern length and may conserve memory during "
            "training then"
         << endl;
    cerr << "\t-W|--wordthreshold  <number>  Word occurrence threshold (secondary threshold): only count patterns in which the words/unigrams occur at least this many times, only "
            "effective when the primary "
         << endl;
    cerr << "\t                              occurrence threshold (-t) is lower than this threshold (default: disabled)" << endl;
    cerr << "\t-p|--prune <number>           Prune all lower-order n-grams below the specified order that are *NOT* subsumed by higher order n-grams (default: 0, disabled). Only "
            "effective when used with -l, usually set to equal values"
         << endl;
    cerr << "\t   --prunesubsumed <number>           Prune all lower-order n-grams below the specified order that are subsumed by higher order n-grams (default: 0, disabled). "
            "Only effective when used with -l, usually set to equal values"
         << endl;
    cerr << "\t-s|--skipgrams                Compute skipgrams (costs extra memory and time)" << endl;
    cerr << "\t-y|--skipthreshold <number>   Occurrence threshold for skipgrams (overrides -t for skipgrams, defaults to -t). Skipgrams occurring less than this will be pruned. "
            "Value must be equal to or higher than -t."
         << endl;
    cerr << "\t-T|--skiptypes <number>       Skip type threshold (for use with -s): only skipgrams with at least x possible types for the skip will be considered, otherwise the "
            "skipgram "
         << endl;
    cerr << "\t                              will be pruned  (default: 2, unindexed models always act as if fixed to 1). Also note that only types that occur above the occurrent "
            "threshold (-t) are counted here! Requires indexed models"
         << endl;
    cerr << "\t-F|--flexgrams S		         Compute flexgrams by abstracting over skipgrams (implies -s). Do not forget the S value!" << endl;
    cerr << "\t-F|--flexgrams <number>       Compute flexgrams (of type X {*} Y only) by using co-occurrence information. The number is the normalised pointwise information "
            "threshold above which to form skipgrams. Only for indexed models."
         << endl;
    ...
    cerr << "\t-I|--constrained               Builds a new model from an input model (-i) and corpus data (-f).  Only patterns present in the input model will be present in the "
            "final model, making"
         << endl;
    cerr << "\t                              the input model the training model and the corpus data the test data. This method uses memory-efficient in-place building, and does "
            "not hold "
         << endl;
    cerr << "\t                              two models (unlike -j). Input model (-i) and or output model (-o) may be indexed or unindexed (-u), this option also allows for "
            "constructing indexed models "
         << endl;
    ...
    cerr << "\t-E|--selfexpand               Expand the loaded pattern model *ON THE SAME CORPUS DATA*, allows you to add, for example, larger order ngrams to an existing model "
            "or skipgrams to an n-gram only model."
         << endl;
    cerr << "\t-e|--expand <number>          Expand the loaded pattern model *ON DIFFERENT CORPUS DATA*, the number is the sentence offset to use in the model (be careful not to "
            "overlap with existing sentence indices!)."
         << endl;
    ...
    cerr << "\t--instantiate                 To use with --print, explicitly prints all instances of skipgrams/flexgrams, regardless of whether they are in the model or not. "
            "(only works for indexed models)"
         << endl;
    ...
    cerr << "\t-g|--relations                Compute and show ALL relationships for the specified patterns (use with -q or -Q). Relationships are: subsumes, subsumed, instances, "
            "templates, leftneighbours,rightneighbours, leftcooc, rightcooc, skipcontent. Only for indexed models."
         << endl;
    cerr << "\t--instances                   Compute and show instances of the specified skipgrams/flexgrams. Use with -q or -Q to constrain to specific patterns; if used "
            "standalone it will print instances of all skipgrams/flexgrams in the model."
         << endl;
    cerr << "\t--skipcontent                 Compute and show the skip content of the specified skipgrams/flexgrams. Use with -q or -Q to constrain to specific patterns; if used "
            "standalone it will apply to all skipgrams/flexgrams in the model."
         << endl;
    cerr << "\t--templates                   Compute and show templates of the specified ngrams, i.e. the skipgrams/flexgrams that it is an instance of. Use with -q or -Q to "
            "constrain to specific patterns; if used standalone it will use all patterns in the model."
         << endl;
    cerr << "\t--subsumes                    Compute and show the patterns that are subsumed by the specified pattern (i.e. sub-parts). Use with -q or -Q to constrain to specific "
            "patterns; if used standalone it will print for all"
         << endl;
    cerr << "\t--subsumed                    Compute and show the patterns that subsume the specified pattern (i.e. larger patterns). Use with -q or -Q to constrain to specific "
            "patterns; if used standalone it will print for all"
         << endl;
    cerr << "\t--leftneighbours              Compute and show the left neighbours of the specified pattern (i.e. the specified pattern is to the right). Use with -q or -Q to "
            "constrain to specific patterns; if used standalone it will print for all"
         << endl;
    cerr << "\t--rightneighbours             Compute and show the right neighbours of the specified pattern (i.e. the specified pattern is to the left). Use with -q or -Q to "
            "constrain to specific patterns; if used standalone it will print for all"
         << endl;

    cerr << "\t--leftcooc                    Compute and show all patterns co-occurring left of the specified pattern within the same sentence/unit (i.e. the specified pattern is "
            "to the right). Use with -q or -Q to constrain to specific patterns; if used standalone it will print for all"
         << endl;
    cerr << "\t--rightcooc                   Compute and show all patterns co-occurring right of the specified pattern within the same sentence/unit (i.e. the specified pattern "
            "is to the left). Use with -q or -Q to constrain to specific patterns; if used standalone it will print for all"
         << endl;
    ...
    cerr << "\t-Y|--npmi <threshold>         Compute and show normalised pointwise mutual information co-occurrence  above the specified threshold [-1,1]. Only for indexed models."
         << endl;
    ...
    cerr << "\tOptions -tlTmxXN can be used to filter the model, -u can be used to remove the index, -j can be used to take the intersection with another model, -F to compute and "
            "add flexgrams"
         << endl;
    cerr << endl;
...
void processquerypattern(ModelType& model, const ClassDecoder* classdecoder, const Pattern& pattern, const string& dorelations, bool doinstantiate) {
    if (!model.has(pattern)) {
        ...
        if (!dorelations.empty())
            model.outputrelations(pattern, *classdecoder, cout, dorelations == "all" ? "" : dorelations);
    }
}

template <class ModelType = IndexedPatternModel<>>
void processquerypatterns(ModelType& model, ClassEncoder* classencoder, const ClassDecoder* classdecoder, const vector<string>& querypatterns, const string& dorelations,
                          bool doinstantiate) {
...
template <class ModelType = IndexedPatternModel<>>
void viewmodel(ModelType& model, ClassDecoder* classdecoder, ClassEncoder* classencoder, bool print, bool report, bool nocoverage, bool histogram, bool query,
               const string& dorelations, bool doinstantiate, bool info, bool printreverseindex, int cooc, double coocthreshold = 0.1) {
...
bool processmodel(const string& inputmodelfile, int inputmodeltype, const string& outputmodelfile, int outputmodeltype, const string& corpusfile, PatternSetModel* constrainbymodel,
                  IndexedCorpus* corpus, const PatternModelOptions& options, bool continued, bool expand, int firstsentence, bool ignoreerrors, const string& inputmodelfile2,
                  ClassDecoder* classdecoder, ClassEncoder* classencoder, bool print, bool report, bool nocoverage, bool histogram, bool query, const string& dorelations,
                  bool doinstantiate, bool info, bool printreverseindex, int cooc, double coocthreshold, bool flexfromskip, const vector<string>& querypatterns) {
    if (!(print || report || histogram || query || info || cooc || printreverseindex || (dorelations != "") || (!querypatterns.empty()) || (!outputmodelfile.empty()))) {
        cerr << "Ooops... You didn't really give me anything to do...that can't be right.. Please study the usage options (-h) again! Did you perhaps forget a --print or "
                "--outputmodel? "
             << endl;
        return false;
    }
    ...
    if (inputmodelfile.empty()) {
        ...
        if (options.DOSKIPGRAMS) {
            if ((inputmodeltype == UNINDEXEDPATTERNMODEL) || (inputmodeltype == UNINDEXEDPATTERNPOINTERMODEL)) {
                ...
                if (flexfromskip)
                    cerr << "WARNING: Can't compute flexgrams from skipgrams on unindexed model" << endl;
            } else {
                if (!inputmodelfile2.empty())
                    cerr << "WARNING: Can not compute skipgrams constrained by " << inputmodelfile2 << "!" << endl;
    ...
    viewmodel<ModelType>(*inputmodel, classdecoder, classencoder, print, report, nocoverage, histogram, query, dorelations, doinstantiate, info, printreverseindex, cooc,
                         coocthreshold);
...
int main(int argc, char* argv[]) {
    ...
    int                  option_index   = 0;
    static struct option long_options[] = {{"twostage", no_argument, 0, '2'},
    ...
    while ((c = getopt_long(argc, argv, "hc:i:j:o:f:t:ul:sT:PRHQDhq:rgGF:S:xXNIVC:Y:L2Zm:vb:y:W:p:Ee:0M", long_options, &option_index)) != -1)
        switch (c) {
            ...
            case 'i': inputmodelfile = optarg; break;
            case 'j': inputmodelfile2 = optarg; break;
            case 'D':
                cerr << "(ENABLING DEBUG MODE)" << endl;
                options.DEBUG = true;
                break;
            case 'R': DOREPORT = true; break;
            case 'f': corpusfile = optarg; break;
            case 't': options.MINTOKENS = atoi(optarg); break;
            case 'T': options.MINSKIPTYPES = atoi(optarg); break;
            case 'y': options.MINTOKENS_SKIPGRAMS = atoi(optarg); break;
            case 'l': options.MAXLENGTH = atoi(optarg); break;
            case 'm': options.MINLENGTH = atoi(optarg); break;
            case 'b': options.MAXBACKOFFLENGTH = atoi(optarg); break;
            case 'W': options.MINTOKENS_UNIGRAMS = atoi(optarg); break;
            case 'p': options.PRUNENONSUBSUMED = atoi(optarg); break;
            case 's': options.DOSKIPGRAMS = true; break;
            case '2': DOTWOSTAGE = true; break;
            case 'o': outputmodelfile = optarg; break;
            case 'u':
            ...
            case 'r':
                ...
                nocoverage = true;
                break;
            case 'g': DORELATIONS = "all"; break;
            case 'Q': DOQUERIER = true; break;
            case 'q': querypatterns.push_back(optarg); break;
            case 'H': DOHISTOGRAM = true; break;
            case 'P': DOPRINT = true; break;
            ...
            case 'C':
                ...
                COOCTHRESHOLD = atof(optarg);
                break;
            case 'x': options.DOREMOVESKIPGRAMS = true; break;
            case 'X': options.DOREMOVEFLEXGRAMS = true; break;
            case 'N': options.DOREMOVENGRAMS = true; break;
            case 'L':
                options.DOPATTERNPERLINE = true;
                outputmodeltype          = UNINDEXEDPATTERNMODEL;
                break;
            case 'I': DOINPLACEREBUILD = true; break;
            case 'V': DOINFO = true; break;
            case 'Z': DOPRINTREVERSEINDEX = true; break;
            case 'E': continued = true; break;
            case 'e':
            ...
            case '0': ignoreerrors = true; break;
            case '?':
                if (optopt == 'c') {
    ...
    for (int stage = 1; stage <= stages; ++stage) {
        ...
        if ((inputmodelfile.empty()) && (corpusfile.empty())) {
            if (argc <= 1) {
                ...
                cerr << "ERROR: No input model (--inputmodel) or corpus data file specified (--datafile|-f), specify at least one. Issue colibri-patternmodeller -h for extensive "
                        "usage options"
                     << classfile << endl;
                exit(2);
            }
        ...
        if (!inputmodelfile.empty()) {
            ...
            if (((inputmodeltype == INDEXEDPATTERNMODEL) || (inputmodeltype == UNINDEXEDPATTERNMODEL)) &&
                ((outputmodeltype == UNINDEXEDPATTERNPOINTERMODEL) || (outputmodeltype == INDEXEDPATTERNPOINTERMODEL))) {
                cerr << "ERROR: Input is a pattern model, can not load as a pattern pointer model!" << endl;
                exit(2);
            }
            if ((!outputmodelfile.empty()) &&
                (((outputmodeltype == INDEXEDPATTERNMODEL) || (outputmodeltype == UNINDEXEDPATTERNMODEL)) &&
                 ((inputmodeltype == UNINDEXEDPATTERNPOINTERMODEL) || (inputmodeltype == INDEXEDPATTERNPOINTERMODEL)))) {
        ...
        if (((outputmodeltype == UNINDEXEDPATTERNMODEL) || (inputmodeltype == UNINDEXEDPATTERNMODEL) || (outputmodeltype == UNINDEXEDPATTERNPOINTERMODEL) ||
             (inputmodeltype == UNINDEXEDPATTERNPOINTERMODEL))) {
            if (!DOINPLACEREBUILD) {
                if (options.DOSKIPGRAMS) {
        ...
        if (DOINPLACEREBUILD) { ///*****************************************************
            ...
            if (outputmodeltype == UNINDEXEDPATTERNMODEL) {
                ...
                if (model.maxlength() > options.MAXLENGTH)
                    options.MAXLENGTH = model.maxlength();
                if (model.minlength() < options.MINLENGTH)
                ...
                viewmodel<PatternModel<uint32_t>>(model, classdecoder, classencoder, DOPRINT, DOREPORT, nocoverage, DOHISTOGRAM, DOQUERIER, DORELATIONS, DOINSTANTIATE, DOINFO,
                                                  DOPRINTREVERSEINDEX, DOCOOC);
                ...
                viewmodel<IndexedPatternModel<>>(model, classdecoder, classencoder, DOPRINT, DOREPORT, nocoverage, DOHISTOGRAM, DOQUERIER, DORELATIONS, DOINSTANTIATE, DOINFO,
                                                 DOPRINTREVERSEINDEX, DOCOOC, COOCTHRESHOLD);
                if (!querypatterns.empty()) {
            ...
            if (outputmodeltype == INDEXEDPATTERNMODEL) {
                processmodel<IndexedPatternModel<>>(inputmodelfile, inputmodeltype, outputmodelfile, outputmodeltype, corpusfile, constrainbymodel, corpus, options, continued,
                                                    expand, firstsentence, ignoreerrors, inputmodelfile2, classdecoder, classencoder, DOPRINT, DOREPORT, nocoverage, DOHISTOGRAM,
                                                    DOQUERIER, DORELATIONS, DOINSTANTIATE, DOINFO, DOPRINTREVERSEINDEX, DOCOOC, COOCTHRESHOLD, DOFLEXFROMSKIP, querypatterns);
            } else if (outputmodeltype == INDEXEDPATTERNPOINTERMODEL) {
                processmodel<IndexedPatternPointerModel<>>(inputmodelfile, inputmodeltype, outputmodelfile, outputmodeltype, corpusfile, constrainbymodel, corpus, options,
                                                           continued, expand, firstsentence, ignoreerrors, inputmodelfile2, classdecoder, classencoder, DOPRINT, DOREPORT,
                                                           nocoverage, DOHISTOGRAM, DOQUERIER, DORELATIONS, DOINSTANTIATE, DOINFO, DOPRINTREVERSEINDEX, DOCOOC, COOCTHRESHOLD,
                                                           DOFLEXFROMSKIP, querypatterns);
            } else if (outputmodeltype == UNINDEXEDPATTERNMODEL) {
                processmodel<PatternModel<uint32_t>>(inputmodelfile, inputmodeltype, outputmodelfile, outputmodeltype, corpusfile, constrainbymodel, corpus, options, continued,
                                                     expand, firstsentence, ignoreerrors, inputmodelfile2, classdecoder, classencoder, DOPRINT, DOREPORT, nocoverage, DOHISTOGRAM,
                                                     DOQUERIER, DORELATIONS, DOINSTANTIATE, DOINFO, DOPRINTREVERSEINDEX, DOCOOC, COOCTHRESHOLD, DOFLEXFROMSKIP, querypatterns);
            } else if (outputmodeltype == UNINDEXEDPATTERNPOINTERMODEL) {
                processmodel<PatternPointerModel<uint32_t>>(inputmodelfile, inputmodeltype, outputmodelfile, outputmodeltype, corpusfile, constrainbymodel, corpus, options,
                                                            continued, expand, firstsentence, ignoreerrors, inputmodelfile2, classdecoder, classencoder, DOPRINT, DOREPORT,
                                                            nocoverage, DOHISTOGRAM, DOQUERIER, DORELATIONS, DOINSTANTIATE, DOINFO, DOPRINTREVERSEINDEX, DOCOOC, COOCTHRESHOLD,
                                                            DOFLEXFROMSKIP, querypatterns);
            }
