...
class PatternModelOptions {
  public:
    ...
    bool
        DOSKIPGRAMS_EXHAUSTIVE; ///< Load/extract skipgrams in an exhaustive fashion? More memory intensive, but the only options for unindexed models (default: false). Use DOSKIPGRAMS instead, this will be automatically used as a fallback whnever EXHAUSTIVE computation is necessary.
...
class PatternSetModel : public PatternSet<uint64_t>, public PatternModelInterface {
  ...
  public:
    ...
    virtual int getmodeltype() const override {
        return PATTERNSETMODEL;
    }
    virtual int getmodelversion() const override {
        return 2;
    }
    ...
    virtual void load(std::istream& f, const PatternModelOptions& options, PatternModelInterface* constrainmodel = NULL) { //load from file
        ...
        if (model_version == 1)
            this->classencodingversion = 1;
        if ((null != 0) ||
            ((model_type != UNINDEXEDPATTERNMODEL) && (model_type != INDEXEDPATTERNMODEL) && (model_type != PATTERNSETMODEL) && (model_type != PATTERNALIGNMENTMODEL))) {
        ...
        if (constrainmodel)
            constrainstore = constrainmodel->getstoreinterface();

        if (options.DEBUG) {
            std::cerr << "Debug enabled, loading PatternModel type " << (int)model_type << ", version " << (int)model_version << ", classencodingversion"
                      << (int)this->classencodingversion << std::endl;
            std::cerr << "Total tokens: " << totaltokens << ", total types: " << totaltypes << std::endl;
            ;
        }
        if (model_type == PATTERNSETMODEL) {
            //reading set
            PatternSet<uint64_t>::read(f, options.MINLENGTH, options.MAXLENGTH, constrainstore, !options.DOREMOVENGRAMS, !options.DOREMOVESKIPGRAMS,
                                       !options.DOREMOVEFLEXGRAMS); //read PatternStore
        } else if (model_type == INDEXEDPATTERNMODEL) {
            //reading from indexed pattern model, ok:
            readmap<IndexedData, IndexedDataHandler>(f, options.MINTOKENS, options.MINLENGTH, options.MAXLENGTH, constrainstore, !options.DOREMOVENGRAMS,
            ...
            readmap<uint32_t, BaseValueHandler<uint32_t>>(f, options.MINTOKENS, options.MINLENGTH, options.MAXLENGTH, constrainstore, !options.DOREMOVENGRAMS,
                                                          !options.DOREMOVESKIPGRAMS, !options.DOREMOVEFLEXGRAMS);
        } else if (model_type == PATTERNALIGNMENTMODEL) {
            //ok:
            readmap<PatternFeatureVectorMap<double>, PatternFeatureVectorMapHandler<double>>(f, options.MINTOKENS, options.MINLENGTH, options.MAXLENGTH, constrainstore,
    ...
    /**
         ...
         * returns zero
         */
    virtual size_t occurrencecount(const Pattern&) override {
        return 0;
    }
    ...
    /**
         ...
         * returns zero
         */
    virtual double frequency(const Pattern&) override {
        return 0;
    }
...
class PatternModel : public MapType, public PatternModelInterface {
  protected:
    ...
    t_matchskipgramhelper
        matchskipgramhelper; ///< Helper structure for finding skipgrams in corpus data: maps the first word of skipgrams to series of mask,length pairs, so skipgrams need not be recomputed exhaustively for each pattern
    ...
    virtual void                               postread(const PatternModelOptions&) {
        ...
        for (iterator iter = this->begin(); iter != this->end(); ++iter) {
            ...
            if (n < minn)
                minn = n;
            if ((!hasskipgrams) && (category == SKIPGRAM))
                hasskipgrams = true;
            if ((!hasflexgrams) && (category == FLEXGRAM))
                hasflexgrams = true;
        }
    }
    ...
    virtual void compute_matchhelpers(bool quiet = false) {
        ...
        for (iterator iter = this->begin(); iter != this->end(); ++iter) {
            ...
            if (!firstword.unknown() && (!firstword.isgap(0))) {
                if ((category == SKIPGRAM) && (doskipgrams)) {
                    bool found = false;
                    for (const auto& iter2 : matchskipgramhelper[firstword]) {
        ...
        for (auto& iter : matchskipgramhelper) {
            iter.second.shrink_to_fit();
        }
        if (!quiet && !matchskipgramhelper.empty())
            std::cerr << "(helper structure has " << matchskipgramhelper.size() << " unigrams mapping to " << skipgramcount << " skipgrams total)" << std::endl;
        if (!quiet && !matchflexgramhelper.empty())
            std::cerr << "(helper structure has " << matchflexgramhelper.size() << " unigrams mapping to " << flexgramcount << " flexgrams total)" << std::endl;
    }
  ...
  public:
    ...
    /**
         ...
         */
    PatternModel(std::istream& f, const PatternModelOptions& options, PatternModelInterface* constrainmodel = NULL, IndexedCorpus* corpus = NULL) {
        totaltokens   = 0;
        totaltypes    = 0;
        maxn          = 0;
        minn          = 999;
    ...
    ~PatternModel() {
        if (reverseindex_internal && reverseindex != NULL)
            delete reverseindex;
    }
    /**
         ...
         * @param corpus Pointer to the loaded corpus, used as a reverse index.
         */
    PatternModel(const std::string& filename, const PatternModelOptions& options, PatternModelInterface* constrainmodel = NULL, IndexedCorpus* corpus = NULL) { //load from file
    ...
    virtual void load(std::istream& f, const PatternModelOptions& options, PatternModelInterface* constrainmodel = NULL) { //load from file
        ...
        if (model_version == 1)
            this->classencodingversion = 1;
        if ((null != 0) ||
            ((model_type != UNINDEXEDPATTERNMODEL) && (model_type != UNINDEXEDPATTERNPOINTERMODEL) && (model_type != INDEXEDPATTERNMODEL) &&
             (model_type != INDEXEDPATTERNPOINTERMODEL) && (model_type != PATTERNALIGNMENTMODEL))) {
        ...
        if (options.DEBUG) {
            std::cerr << "Debug enabled, loading PatternModel type " << (int)model_type << ", version " << (int)model_version
        ...
        if ((model_type == UNINDEXEDPATTERNPOINTERMODEL) || (model_type == INDEXEDPATTERNPOINTERMODEL)) {
            ...
            if (options.DEBUG)
                std::cerr << "Reading corpus data" << std::endl;
            uint64_t corpussize;
            ...
            if (options.DEBUG)
                std::cerr << "(read " << corpussize << " bytes)" << std::endl;
        }
        f.read((char*)&totaltokens, sizeof(uint64_t));
        f.read((char*)&totaltypes, sizeof(uint64_t));
        ...
        if (((model_type == INDEXEDPATTERNMODEL) && (this->getmodeltype() == UNINDEXEDPATTERNMODEL)) ||
            ...
            MapType::template read<IndexedData, IndexedDataHandler, PatternType>(f, options.MINTOKENS, options.MINLENGTH, options.MAXLENGTH, constrainstore,
                                                                                 !options.DOREMOVENGRAMS, !options.DOREMOVESKIPGRAMS, !options.DOREMOVEFLEXGRAMS, options.DORESET,
                                                                                 options.DEBUG);
            ...
            MapType::template read<uint32_t, BaseValueHandler<uint32_t>, PatternType>(f, options.MINTOKENS, options.MINLENGTH, options.MAXLENGTH, constrainstore,
                                                                                      !options.DOREMOVENGRAMS, !options.DOREMOVESKIPGRAMS, !options.DOREMOVEFLEXGRAMS,
            ...
            MapType::template read<uint32_t, BaseValueHandler<uint32_t>, PatternPointer>(f, options.MINTOKENS, options.MINLENGTH, options.MAXLENGTH, constrainstore,
                                                                                         !options.DOREMOVENGRAMS, !options.DOREMOVESKIPGRAMS, !options.DOREMOVEFLEXGRAMS,
                                                                                         options.DORESET, options.DEBUG);
            ...
            MapType::template read<IndexedData, IndexedDataHandler, PatternPointer>(f, options.MINTOKENS, options.MINLENGTH, options.MAXLENGTH, constrainstore,
                                                                                    !options.DOREMOVENGRAMS, !options.DOREMOVESKIPGRAMS, !options.DOREMOVEFLEXGRAMS,
                                                                                    options.DORESET, options.DEBUG);
        } else if (model_type == PATTERNALIGNMENTMODEL) {
    ...
    /**
         ...
         */
    virtual void train(std::istream* in, const PatternModelOptions& in_options, PatternModelInterface* constrainbymodel = NULL, PatternSet<>* filter = NULL, bool continued = false,
                       uint32_t firstsentence = 1, bool ignoreerrors = false) {
        PatternModelOptions options = in_options; // we need a copy of the options
        if (options.MINTOKENS == -1)
            options.MINTOKENS = 2;
        if (options.MINTOKENS == 0)
            options.MINTOKENS = 1;
        if (options.MINTOKENS_SKIPGRAMS < options.MINTOKENS)
            options.MINTOKENS_SKIPGRAMS = options.MINTOKENS;
        ...
        if (filter != NULL) {
            if (filter->size() == 0) { //cython will pass empty sets
                ...
                for (auto iter = filter->begin(); iter != filter->end(); ++iter) {
                    ...
                    if (filterhasngrams && filterhasskipgrams)
                        break;
                }
            }
        }
        ...
        if (!options.QUIET) {
            ...
            if (constrainbymodel != NULL)
                std::cerr << ", constrained by another model";
            std::cerr << ", occurrence threshold: " << options.MINTOKENS;
            if (iter_unigramsonly)
                std::cerr << ", secondary word occurrence threshold: " << options.MINTOKENS_UNIGRAMS;
            if (data_version < 2)
                std::cerr << ", class encoding version: " << (int)data_version;
            std::cerr << std::endl;
            if (filterhasngrams) {
                std::cerr << "Filter with ngrams provided, only patterns that either match a filtered pattern or contain a smaller filtered pattern will be included..."
                          << std::endl;
            }
        ...
        if (constrainbymodel != NULL) {
            if ((options.DOSKIPGRAMS) && (!options.DOSKIPGRAMS_EXHAUSTIVE)) {
                if (constrainbymodel != this) {
                    ...
                    if (!options.QUIET)
                        std::cerr << "WARNING: Skipgrams will be extracted exhaustively on the basis of the ngrams found; the constraint model will be applied only afterwards. "
                                     "This implies some skipgrams in the constraint model that are present may be missed, and it will not be most efficient. Use in-place "
                                     "rebuilding of your constraint model instead."
                                  << std::endl;
                    if (options.MAXLENGTH >= 31)
                        std::cerr << "WARNING: No maximum pattern length set or maximum pattern length is very high! This will drastically slow down skipgram computation"
                                  << std::endl;
                }
            }
        }
        ...
        if (constrainbymodel == this)
            prevsize = 0; //going over same model
        int      backoffn    = 0;
        ...
        for (int n = 1; n <= options.MAXLENGTH; ++n) {
            ...
            while (((reverseindex != NULL) && (sentence < sentences)) || ((reverseindex == NULL) && (in != NULL) && (!in->eof()))) {
                ...
                if (reverseindex == NULL)
                    linepattern = new Pattern(*in, false, data_version);
                ...
                if (options.DEBUG)
                    std::cerr << "Processing line " << sentence << ", size (tokens) " << linesize << " (bytes) " << line.bytesize() << ", n=" << n << std::endl;
                if (linesize == 0) {
                    //skip empty lines
                    continue;
                }
                //count total tokens
                if ((n == 1) && (!continued))
                    totaltokens += linesize;

                ngrams.clear();
                ngrams.reserve(linesize);
                if (options.DEBUG)
                    std::cerr << "   (container ready)" << std::endl;

                if (options.DOPATTERNPERLINE) {
                    if (linesize > (unsigned int)options.MAXLENGTH)
                        continue;
                    ...
                    if (iter_unigramsonly) {
                        ...
                        singlepass     = true;
                        int min_length = options.MINLENGTH;
                        if (continued)
                            min_length = this->maxn + 1;
                        line.subngrams(
                            ngrams, min_length,
                ...
                if (options.DEBUG)
                    std::cerr << "\t" << ngrams.size() << " ngrams in line" << std::endl;
                ...
                for (auto const& iter : ngrams) {
                    ...
                    try {
                        ...
                        if (!skipgramsonly) {
                            ...
                            if ((constrainbymodel != NULL) && (!iter_unigramsonly) && (!constrainbymodel->has(iter.first)))
                            ...
                            if ((found) && (filter != NULL) && (constrainbymodel == NULL)) {
                                ...
                                if (((options.MINTOKENS > 1) && (n >= options.MINLENGTH)) || ((options.MINTOKENS == 1) && (iter.first.n() >= (unsigned int)options.MINLENGTH))) {
                                    ...
                                    if ((!matchfilter) && (filterhasskipgrams)) {
                                        for (auto iter2 = filter->begin(); iter2 != filter->end(); ++iter2) {
                            ...
                            if ((found) && (!skipgramsonly)) {
                                if (options.DEBUG)
                                    std::cerr << "\t\tAdding @" << ref.sentence << ":" << ref.token << " n=" << iter.first.n() << " category=" << (int)iter.first.category()
                                              << std::endl;
                                add(iter.first, ref);
                            }
                        }
            ...
            if (!iter_unigramsonly) {
                int foundngrams = this->size() - foundskipgrams - prevsize;
                ...
                if (!options.QUIET)
                    std::cerr << " Found " << foundngrams << " ngrams...";
                ...
                if ((!continued) && ((constrainbymodel == NULL) or (constrainbymodel == this))) {
                    if ((options.MINTOKENS > 1) && (n == 1)) {
                        ...
                        if (!options.QUIET)
                            std::cerr << " computing total word types prior to pruning...";
                ...
                if (singlepass) {
                    ...
                    if ((!options.DOSKIPGRAMS) && (!options.DOSKIPGRAMS_EXHAUSTIVE) && (n - 1 >= 1) && ((n - 1) < options.MINLENGTH) && (n - 1 != options.MAXBACKOFFLENGTH) &&
                        ...
                        this->prune(-1, n - 1);
                        if (!options.QUIET)
                            std::cerr << " (pruned last iteration due to minimum length) ";
                    }
                }
                if (!options.QUIET)
                ...
                if (foundskipgrams) {
                    ...
                    if (prunedextra && !options.QUIET)
                        std::cerr << " plus " << prunedextra << " extra skipgrams..";
                    pruned += prunedextra;
                }
                if (!options.QUIET)
                ...
                if ((n == 1) && (options.MINLENGTH == 1))
                    skipunigrams = true; //prevent double counting of unigrams
        ...
        if (options.PRUNENONSUBSUMED) {
            if (!options.QUIET)
                std::cerr << "Pruning non-subsumed n-grams" << std::endl;
            int begin_n = options.PRUNENONSUBSUMED;
            if ((begin_n > options.MAXLENGTH))
                begin_n = options.MAXLENGTH;
            for (int n = begin_n; n > 1; n--) {
                ...
                if (!options.QUIET)
                    std::cerr << " pruned " << prunednonsubsumed << " non-subsumed " << (n - 1) << "-grams" << std::endl;
            }
        }
        if (options.PRUNESUBSUMED) {
            if (!options.QUIET)
                std::cerr << "Pruning subsumed n-grams" << std::endl;
            ...
            for (int n = 2; n <= end_n; ++n) {
                ...
                if (!options.QUIET)
                    std::cerr << " pruned " << prunedsubsumed << " subsumed " << (n - 1) << "-grams" << std::endl;
            }
        }
        if ((options.MINLENGTH > 1) && (options.DOSKIPGRAMS || options.DOSKIPGRAMS_EXHAUSTIVE)) {
            ...
            if (!options.QUIET)
                std::cerr << " pruned " << pruned << " patterns below minimum length (" << options.MINLENGTH << ")" << std::endl;
        }
    ...
    /**
         ...
         * @param constrainbymodel Pointer to another pattern model which should be used to constrain the training of this one, only patterns also occurring in the other model will be included. Defaults to NULL (no constraining)
         */
    virtual void train(const std::string& filename, const PatternModelOptions& options, PatternModelInterface* constrainbymodel = NULL, PatternSet<>* filter = NULL,
                       bool continued = false, uint32_t firstsentence = 1, bool ignoreerrors = false) {
    ...
    /**
         ...
         */
    virtual int computeskipgrams(const PatternPointer& pattern, int mintokens = 2, const IndexReference* singleref = NULL, const IndexedData* multiplerefs = NULL,
                                 PatternModelInterface* constrainbymodel = NULL, std::vector<PatternPointer>* targetcontainer = NULL, const bool exhaustive = false,
        ...
        if (gapmasks[n].empty())
            gapmasks[n] = compute_skip_configurations(n, maxskips);

        if (DEBUG)
            std::cerr << "Computing skipgrams (" << gapmasks[n].size() << " permutations)" << std::endl;

        //loop over all possible gap configurations
        for (const auto& iter2 : gapmasks[n]) {
            if (iter2 == 0)
                continue; //precaution (doesn't really happen anyway, but better safe than sorry)
            ...
            try {
                ...
                if ((constrainbymodel != NULL) && (!constrainbymodel->has(skipgram)))
                    continue;
                ...
                if ((mintokens != 1) && (constrainbymodel == NULL)) {
                    ...
                    for (const auto& subpattern : subngrams) {
                    ...
                    if (check_extra) {
                        ...
                        for (const auto& iter3 : gapconfiguration) {
            ...
            test.out();
                    ...
                    if (targetcontainer == NULL) {
                        ...
                        if (singleref != NULL) {
                            ...
                            for (auto refiter = multiplerefs->begin(); refiter != multiplerefs->end(); ++refiter) {
            ...
            } catch (const InternalError& e) { std::cerr << "IGNORING ERROR and continuing with next skipgram" << std::endl; }
        }
    ...
    /**
         ...
         */
    virtual int computeskipgrams(const PatternPointer& pattern, PatternModelOptions& options, const IndexReference* singleref = NULL, const IndexedData* multiplerefs = NULL,
                                 PatternModelInterface* constrainbymodel = NULL, const bool exhaustive = false) { //backward compatibility
        if (options.MINTOKENS_SKIPGRAMS < options.MINTOKENS)
            options.MINTOKENS_SKIPGRAMS = options.MINTOKENS;
    ...
    virtual void trainskipgrams_selfconstrained(const PatternModelOptions& _options) {
        ...
        if (!options.QUIET)
            std::cerr << "Finding skipgrams/flexgrams matching preloaded constraints, occurrence threshold " << options.MINTOKENS << " ..." << std::endl;
    ...
    /**
         ...
         */
    size_t types() override {
        if ((totaltypes == 0) && (!this->data.empty()))
            totaltypes = this->totalwordtypesingroup(0, 0);
        return totaltypes;
    }
    ...
    std::unordered_set<PatternPointer> getreverseindex(const IndexReference ref, int occurrencecount = 0, int category = 0, unsigned int size = 0, int = 3) {
        ...
        const unsigned int min_n = this->minlength();
        const unsigned int max_n = this->maxlength();
        for (unsigned int n = min_n; ref.token + n <= sl && n <= max_n; ++n) {
            if ((size == 0) || (n == size)) {
                try {
                        ...
                        if (iter != matchskipgramhelper.end()) {
                            for (const auto& iter2 : iter->second) {
                                if (n == iter2.second) {
                                    ...
                                    if (((occurrencecount == 0) && this->has(skipgram)) ||
                                        ((occurrencecount != 0) && (this->occurrencecount(skipgram) >= (unsigned int)occurrencecount))) {
    ...
    std::vector<std::pair<IndexReference, PatternPointer>> getreverseindex_left(const IndexReference ref, int occurrencecount = 0, int category = 0, unsigned int size = 0) {
        ...
        for (unsigned int i = 0; i < ref.token; ++i) {
            const IndexReference               ref2      = IndexReference(ref.sentence, i);
            std::unordered_set<PatternPointer> tmpresult = this->getreverseindex(ref, occurrencecount, category, size);
            for (const auto& pattern : tmpresult) {
                result.push_back(std::pair<IndexReference, PatternPointer>(ref2, pattern));
            }
        }
        return result;
    ...
    virtual void computecoveragestats(int category = 0, int n = 0) {
        ...
        for (auto iterc = cache_categories.begin(); iterc != cache_categories.end(); ++iterc) {
            if ((category == 0) || (*iterc == category)) {
                for (auto itern = cache_n.begin(); itern != cache_n.end(); ++itern) {
                    if (((n == 0) || (*itern == n)) && (!cache_processed[*iterc][*itern])) {
                        std::unordered_set<PatternType> p_types;
                        PatternModel::iterator          iter = this->begin();
                        while (iter != this->end()) {
                            const PatternType pattern = iter->first;
                            const int         pn      = (int)pattern.n();
                            if ((pn == 1) && (*itern <= 1) && ((*iterc == 0) || (pattern.category() == *iterc))) {
                                p_types.insert(pattern);
                            } else {
                        ...
                        cache_grouptotalwordtypes[*iterc][*itern] += p_types.size();
    ...
    unsigned int totalpatternsingroup(int category, int n) {
        //category and n can be set to 0 to loop over all
        if ((cache_grouptotalpatterns.empty()) && (!this->data.empty()))
            this->computestats();
    ...
    unsigned int totalwordtypesingroup(int category, int n) {
        ...
        //category and n can be set to 0 to loop over all
        if ((cache_grouptotalwordtypes.empty()) && (!this->data.empty()))
            this->computecoveragestats(category, n);
    ...
    unsigned int prune(int threshold, int _n = 0, int category = 0) {
        ...
        while (iter != this->end()) {
            const PatternType pattern = iter->first;
            if (((_n == 0) || (pattern.n() == (unsigned int)_n)) && ((category == 0) || (pattern.category() == category)) &&
   ...
   */
    virtual void print(std::ostream& out, ClassDecoder& decoder, bool instantiate = false) {
        ...
        if (haveoutput) {
            ...
            std::cerr << " - TOKENS     : The maximum number of tokens in the corpus that this pattern covers. *THIS IS JUST A MAXIMUM PROJECTION* rather than an exact number "
                         "because your model is not indexed"
                      << std::endl;
   ...
   */
    ...
    /**
         ...
         */
    void printpattern(std::ostream& out, const ClassDecoder& decoder, const Pattern& pattern, bool instantiate = false,
    ...
    void histogram(std::map<unsigned int, unsigned int>& hist, unsigned int threshold = 0, unsigned int cap = 0, int category = 0, unsigned int size = 0) {
        for (PatternModel::iterator iter = this->begin(); iter != this->end(); ++iter) {
            const PatternType pattern = iter->first;
            if (((category != 0) && (pattern.category() != category)) || ((size != 0) && (size != pattern.size())))
                continue;
   ...
   */
    void report(std::ostream& OUT, bool nocoverage = false) {
        ...
        OUT << "----------------------------------" << std::endl;
        OUT << "                          " << std::setw(15) << "PATTERNS" << std::setw(15) << "TOKENS" << std::setw(15) << "COVERAGE" << std::setw(15) << "TYPES" << std::setw(15)
            << std::endl;
        ...
        if (!nocoverage) {
            ...
            if (coveredtokens > this->tokens())
                coveredtokens = this->tokens();
            ...
            OUT << "Uncovered:                " << std::setw(15) << "-" << std::setw(15) << uncoveredtokens << std::setw(15) << uncoveredtokens / (double)this->tokens()
                << std::setw(15) << this->types() - coveredtypes << std::endl;
            OUT << "Covered:                  " << std::setw(15) << this->size() << std::setw(15) << coveredtokens << std::setw(15) << coveredtokens / (double)this->tokens()
                << std::setw(15) << coveredtypes << std::endl
                << std::endl;
        } else {
            OUT << std::endl;
        }
    ...
    PatternSet<uint64_t> extractset(int minlength = 1, int maxlength = 1) {
        ...
        for (PatternModel::iterator iter = this->begin(); iter != this->end(); ++iter) {
            ...
            if ((patternlength >= minlength) && (patternlength <= maxlength)) {
                ...
                for (const auto& pattern2 : subngrams) {
    ...
    virtual t_relationmap getsubchildren(const PatternPointer&, unsigned int = 0, int = 0, unsigned int = 0) {
        return t_relationmap();
    } //does nothing for unindexed models
    virtual t_relationmap getsubparents(const PatternPointer&, unsigned int = 0, int = 0, unsigned int = 0) {
        return t_relationmap();
    } //does nothing for unindexed models
    virtual t_relationmap gettemplates(const PatternPointer&, unsigned int = 0) {
        return t_relationmap();
    } //does nothing for unindexed models
    virtual t_relationmap getinstances(const PatternPointer&, unsigned int = 0) {
        return t_relationmap();
    } //does nothing for unindexed models
    virtual t_relationmap getskipcontent(const PatternPointer&) {
        return t_relationmap();
    } //does nothing for unindexed models
    virtual t_relationmap getleftneighbours(const PatternPointer&, unsigned int = 0, int = 0, unsigned int = 0, unsigned int = 0) {
        return t_relationmap();
    } //does nothing for unindexed models
    virtual t_relationmap getrightneighbours(const PatternPointer&, unsigned int = 0, int = 0, unsigned int = 0, unsigned int = 0) {
        return t_relationmap();
    } //does nothing for unindexed models
    virtual t_relationmap_double getnpmi(const Pattern&, double) {
        return t_relationmap_double();
    } //does nothing for unindexed models
    virtual int computeflexgrams_fromskipgrams() {
        return 0;
    } //does nothing for unindexed models
    virtual int computeflexgrams_fromcooc(double) {
        return 0;
    } //does nothing for unindexed models
...
class IndexedPatternModel : public PatternModel<IndexedData, IndexedDataHandler, MapType, PatternType> {
  protected:
    void postread(const PatternModelOptions&) override {
        for (auto iter = this->begin(); iter != this->end(); ++iter) {
            ...
            if (n < this->minn)
                this->minn = n;
            if ((!this->hasskipgrams) && (category == SKIPGRAM))
                this->hasskipgrams = true;
            if ((!this->hasflexgrams) && (category == FLEXGRAM))
                this->hasflexgrams = true;
        }
    }
  ...
  public:
    ...
    * Begin a new pattern model, optionally pre-setting a reverseindex.
    */
    IndexedPatternModel(IndexedCorpus* corpus = NULL) : PatternModel<IndexedData, IndexedDataHandler, MapType, PatternType>() {
    ...
    */
    ...
    void train(std::istream* in, const PatternModelOptions& options, PatternModelInterface* constrainbymodel = NULL, PatternSet<>* filter = NULL, bool continued = false,
               uint32_t firstsentence = 1, bool ignoreerrors = false) override {
    ...
    void train(const std::string& filename, const PatternModelOptions& options, PatternModelInterface* constrainbymodel = NULL, PatternSet<>* filter = NULL, bool continued = false,
               uint32_t firstsentence = 1, bool ignoreerrors = false) override {
        if ((options.DOSKIPGRAMS) && (this->reverseindex == NULL)) {
            std::cerr << "ERROR: You must specify a reverse index if you want to train skipgrams (or train skipgrams exhaustively)" << std::endl;
            throw InternalError();
   ...
   */
    ...
    */
     ...
     */
    void trainskipgrams(const PatternModelOptions& _options, PatternModelInterface* constrainbymodel = NULL) override {
        PatternModelOptions options = _options; // need a copy to set MINTOKENS
        if (options.MINTOKENS == -1)
            options.MINTOKENS = 2;
        ...
        if (constrainbymodel == this) {
            ...
            if (!options.QUIET)
                std::cerr << "Finding skipgrams on the basis of extracted n-grams..." << std::endl;
            for (int n = 3; n <= options.MAXLENGTH; ++n) {
                if (this->gapmasks[n].empty())
                    this->gapmasks[n] = compute_skip_configurations(n, options.MAXSKIPS);
                if (!options.QUIET)
                    std::cerr << "Counting " << n << "-skipgrams" << std::endl;
                unsigned int foundskipgrams = 0;
                for (typename MapType::iterator iter = this->begin(); iter != this->end(); ++iter) {
                    ...
                    if (((int)pattern.n() == n) && (pattern.category() == NGRAM))
                        foundskipgrams += this->computeskipgrams(pattern, options, NULL, &multirefs, constrainbymodel, false);
                }
                if (!foundskipgrams) {
                    std::cerr << " None found" << std::endl;
                    break;
                ...
                if (!options.QUIET)
                    std::cerr << " Found " << foundskipgrams << " skipgrams...";
    ...
    t_relationmap gettemplates(const Pattern& pattern, unsigned int occurrencethreshold = 0) {
        ...
        if (occurrencethreshold > 0)
            this->prunerelations(templates, occurrencethreshold);
        return templates;
    }
    ...
    t_relationmap getinstances(const Pattern& pattern, unsigned int occurrencethreshold = 0) {
        ...
        if (occurrencethreshold > 0)
            this->prunerelations(instances, occurrencethreshold);
        return instances;
    }
    ...
    t_relationmap getsubchildren(const PatternPointer& pattern, unsigned int occurrencethreshold = 0, int category = 0, unsigned int size = 0) override {
        ...
        for (IndexedData::iterator iter = data->begin(); iter != data->end(); ++iter) {
            ...
            for (int i = ref.token; i < ref.token + _n; ++i) {
                const IndexReference first   = IndexReference(ref.sentence, i);
                int                  maxsubn = _n - (i - ref.token);

                //std::cerr << "Begin " << first.sentence << ":" << first.token << ",<< std::endl;
        ...
        if (occurrencethreshold > 0)
            this->prunerelations(subchildren, occurrencethreshold);
        return subchildren;
    }
    ...
    t_relationmap getsubparents(const PatternPointer& pattern, unsigned int occurrencethreshold = 0, int category = 0, unsigned int size = 0) override {
        ...
        for (IndexedData::iterator iter = data->begin(); iter != data->end(); ++iter) {
            ...
            for (const auto& iter2 : rindex) {
                if ((iter2.first.sentence != ref.sentence) || (iter2.first.token > ref.token))
                    break;
                const PatternPointer candidate = iter2.second;

                int                  minsubsize = _n + (ref.token - iter2.first.token);

                if (((int)candidate.n() >= minsubsize) && (candidate != pattern) && ((occurrencethreshold == 0) || (this->occurrencecount(candidate) >= occurrencethreshold)) &&
                    ((category == 0) || (candidate.category() == category)) && ((size == 0) || (candidate.n() == size))) {
        ...
        if (occurrencethreshold > 0)
            this->prunerelations(subsumes, occurrencethreshold);
        return subsumes;
    }
    ...
    * @return a relation map
    */
    t_relationmap getleftneighbours(const PatternPointer& pattern, unsigned int occurrencethreshold = 0, int category = 0, unsigned int size = 0,
                                    unsigned int cutoff = 0) override {
        if ((this->reverseindex == NULL) || (this->reverseindex->empty())) {
            std::cerr << "ERROR: No reverse index present" << std::endl;
            throw InternalError();
        }
        ...
        for (IndexedData::iterator iter = data->begin(); iter != data->end(); ++iter) {
            ...
            for (const auto& iter2 : rindex) {
                ...
                if ((ref2.token + neighbour.n() == ref.token) && ((occurrencethreshold == 0) || (this->occurrencecount(neighbour) >= occurrencethreshold)) &&
                    ((category == 0) || (neighbour.category() == category)) && ((size == 0) || (neighbour.n() == size))) {
                    ++neighbours[neighbour];
                    if ((cutoff > 0) && (neighbours.size() >= cutoff))
                        break;
                } else if ((ref2.token > ref.token) || (ref2.sentence > ref.sentence))
                    break;
            }
            if ((cutoff > 0) && (neighbours.size() >= cutoff))
                break;
        ...
        if (occurrencethreshold > 0)
            this->prunerelations(neighbours, occurrencethreshold);
        return neighbours;
    }
    ...
    * @return a relation map
    */
    t_relationmap getrightneighbours(const PatternPointer& pattern, unsigned int occurrencethreshold = 0, int category = 0, unsigned int size = 0,
                                     unsigned int cutoff = 0) override {
        if ((this->reverseindex == NULL) || (this->reverseindex->empty())) {
            std::cerr << "ERROR: No reverse index present" << std::endl;
            throw InternalError();
        ...
        for (IndexedData::iterator iter = data->begin(); iter != data->end(); ++iter) {
            ...
            for (const auto& neighbour : rindex) {
                if (((occurrencethreshold == 0) || (this->occurrencecount(neighbour) >= occurrencethreshold)) && ((category == 0) || (neighbour.category() == category)) &&
    ...
    */
    void computecoveragestats(int category = 0, int n = 0) override {
        ...
        if ((this->cache_grouptotal.empty()) && (!this->data.empty()))
            this->computestats();
        ...
        for (const auto& cat : this->cache_categories) {
            if ((category == 0) || (cat == category)) {
                for (const auto& itern : this->cache_n) {
                    if (((n == 0) || (itern == n)) && (!this->cache_processed[cat][itern])) {
                        std::unordered_set<PatternPointer>                                        types;
                        std::set<IndexReference>                                                  tokens;
                        typename PatternModel<IndexedData, IndexedDataHandler, MapType>::iterator iter = this->begin();
                        while (iter != this->end()) {
                            const PatternPointer pattern = iter->first;
                            const int            p_n     = pattern.n();
                            if ((p_n == 1) && (itern <= 1) && ((cat == 0) || (pattern.category() == cat))) {
                            ...
                            if (((itern == 0) || (n == itern)) && ((cat == 0) || (pattern.category() == cat))) {
                                IndexedData* data = this->getdata(pattern);
                        ...
                        this->cache_grouptotalwordtypes[cat][itern] += types.size();
                        this->cache_grouptotaltokens[cat][itern] += tokens.size();
                        this->cache_processed[cat][itern] = true;
                    }
                }
            }
        }
    ...
    * @return a relation map
    */
    t_relationmap getrightcooc(const PatternPointer& pattern, unsigned int occurrencethreshold = 0, int category = 0, unsigned int size = 0,
                               std::vector<std::pair<IndexReference, IndexReference>>* matches = NULL) {
        if ((this->reverseindex == NULL) || (this->reverseindex->empty())) {
            std::cerr << "ERROR: No reverse index present" << std::endl;
            throw InternalError();
        }
        ...
        for (IndexedData::iterator iter = data->begin(); iter != data->end(); ++iter) {
            ...
            for (const auto& iter2 : rindex) {
                const IndexReference ref2      = iter2.first;
                const PatternPointer neighbour = iter2.second;
                if ((ref2.token > ref.token + _n) && ((occurrencethreshold == 0) || (this->occurrencecount(neighbour) >= occurrencethreshold)) &&
                    ((category == 0) || (neighbour.category() == category)) && ((size == 0) || (neighbour.n() == size))) {
                    ++cooc[neighbour];
                    if (matches != NULL)
                        matches->push_back(std::pair<IndexReference, IndexReference>(ref, ref2));
                }
            }
        }
        if (occurrencethreshold > 0)
            this->prunerelations(cooc, occurrencethreshold);
        return cooc;
    }
    ...
    t_relationmap getleftcooc(const PatternPointer& pattern, unsigned int occurrencethreshold = 0, int category = 0, unsigned int size = 0) {
        ...
        for (IndexedData::iterator iter = data->begin(); iter != data->end(); ++iter) {
            ...
            for (const auto& iter2 : rindex) {
                ...
                if ((ref2.token + _n < ref.token) && ((occurrencethreshold == 0) || (this->occurrencecount(neighbour) >= occurrencethreshold)) &&
                    ((category == 0) || (neighbour.category() == category)) && ((size == 0) || (neighbour.n() == size))) {
                    ++cooc[neighbour];
                }
            }
    ...
    */
    t_relationmap getcooc(const PatternPointer& pattern, unsigned int occurrencethreshold = 0, int category = 0, unsigned int size = 0, bool ordersignificant = false) {
        ...
        for (IndexedData::iterator iter = data->begin(); iter != data->end(); ++iter) {
            ...
            for (const auto& iter2 : rindex) {
                const IndexReference ref2      = iter2.first;
                const PatternPointer neighbour = iter2.second;
                if ((ordersignificant) && (neighbour.pattern() < pattern))
                    continue;
                const int _n2 = neighbour.n();
                if (((ref2.token + _n2 < ref.token) || (ref2.token > ref.token + _n)) &&
                    ((occurrencethreshold == 0) || (this->occurrencecount(neighbour) >= occurrencethreshold)) && ((category == 0) || (neighbour.category() == category)) &&
                    ((size == 0) || (neighbour.n() == size))) {
                    ++cooc[neighbour];
                }
            }
     ...
     */
    void outputrelations(const PatternPointer& pattern, const t_relationmap& relations, const ClassDecoder& classdecoder, std::ostream& OUT,
        ...
        for (const auto& iter : relations) {
            ...
            OUT << "\t" << pattern_s << "\t" << label << "\t" << pattern2.tostring(classdecoder) << "\t" << iter.second << "\t" << iter.second / total_f << "\t"
...
class PatternPointerModel : public PatternModel<ValueType, ValueHandler, MapType, PatternPointer> {
  public:
    ...
    explicit PatternPointerModel(IndexedCorpus* corpus) : PatternModel<ValueType, ValueHandler, MapType, PatternPointer>() {
        this->model_type    = this->getmodeltype();
        this->model_version = this->getmodelversion();
        if (corpus) {
            this->reverseindex = corpus;
    ...
    int getmodeltype() const override {
        return UNINDEXEDPATTERNPOINTERMODEL;
    }
    int getmodelversion() const override {
        return 2;
    }
...
class IndexedPatternPointerModel : public IndexedPatternModel<MapType, PatternPointer> {
  public:
    ...
    explicit IndexedPatternPointerModel(IndexedCorpus* corpus) : IndexedPatternModel<MapType, PatternPointer>() {
        this->model_type    = this->getmodeltype();
        this->model_version = this->getmodelversion();
        if (corpus) {
            this->reverseindex = corpus;
    ...
    int getmodeltype() const override {
        return INDEXEDPATTERNPOINTERMODEL;
    }
    int getmodelversion() const override {
        return 2;
    }
...
void   comparemodels_loglikelihood(std::vector<PatternModel<uint32_t>*>& models, PatternMap<double>* resultmap, bool conjunctiononly = false, std::ostream* output = NULL,
                                   const ClassDecoder* classdecoder = NULL);
