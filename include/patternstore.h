...
class IndexedCorpus {
   ...
   */
    ...
    /**
         ...
         */
    PatternPointer getinstance(const IndexReference& begin, const Pattern& pattern, PatternCategory resultcategory = NGRAM) const {
        return findpattern(begin, pattern, resultcategory);
    } //alias, to be consistent with Cython names
    ...
    } //returns the number of sentences (1-indexed)
    ...
    iterator find(const IndexReference& ref) {
        ...
        } catch (const KeyError& e) { return end(); }
    }
    /**
...
class PatternMapStore : public PatternStore<ContainerType, ReadWriteSizeType, PatternType> {
  ...
  public:
    ...
    void read(std::istream& in, int MINTOKENS = 0, int MINLENGTH = 0, int MAXLENGTH = 999999, const PatternStoreInterface* constrainstore = NULL, bool DONGRAMS = true,
              bool DOSKIPGRAMS = true, bool DOFLEXGRAMS = true, bool DORESET = false, bool DEBUG = false) {
        ...
        reserve(s);
        if (DEBUG)
            std::cerr << "Reading " << s << " patterns, classencodingversion=" << (int)this->classencodingversion << ", @corpusstart=" << (size_t)this->corpusstart << std::endl;
        if (MINTOKENS == -1)
            MINTOKENS = 0;
        for (ReadWriteSizeType i = 0; i < s; ++i) {
            ...
            if (!DONGRAMS || !DOSKIPGRAMS || !DOFLEXGRAMS) {
                ...
                if ((!DONGRAMS && c == NGRAM) || (!DOSKIPGRAMS && c == SKIPGRAM) || (!DOFLEXGRAMS && c == FLEXGRAM))
                    continue;
            }
            const int n = p.size();
            if (DEBUG)
                std::cerr << "Read pattern #" << (i + 1) << ", size=" << n << ", valuehandler=" << readvaluehandler.id()
                          << ", classencodingversion=" << (int)this->classencodingversion;
            ...
            if (n >= MINLENGTH && n <= MAXLENGTH) {
                if ((readvaluehandler.count(readvalue) >= (unsigned int)MINTOKENS) && ((constrainstore == NULL) || (constrainstore->has(p)))) {
                    ...
                    if ((void*)convertedvalue != (void*)&readvalue)
    ...
    /**
         ...
         */
    void read(const std::string& filename, int MINTOKENS = 0, int MINLENGTH = 0, int MAXLENGTH = 999999, PatternStoreInterface* constrainstore = NULL, bool DONGRAMS = true,
              bool DOSKIPGRAMS = true, bool DOFLEXGRAMS = true, bool DORESET = false, bool DEBUG = false) { //no templates for this one, easier on python/cython
...
class PatternSet : public PatternStore<t_patternset, ReadWriteSizeType, Pattern> {
   ...
   */
    ...
    /**
         ...
         */
    void read(std::istream& in, int MINLENGTH = 0, int MAXLENGTH = 999999, const PatternStoreInterface* constrainstore = NULL, bool DONGRAMS = true, bool DOSKIPGRAMS = true,
    ...
    void readmap(std::istream& in, int MINTOKENS = 0, int MINLENGTH = 0, int MAXLENGTH = 999999, const PatternStoreInterface* constrainstore = NULL, bool DONGRAMS = true,
                 bool DOSKIPGRAMS = true, bool DOFLEXGRAMS = true) {
        ReadValueHandler  readvaluehandler = ReadValueHandler();
        ReadWriteSizeType s; //read size:
        in.read((char*)&s, sizeof(ReadWriteSizeType));
        reserve(s);
...
class PatternMap : public PatternMapStore<std::unordered_map<Pattern, ValueType>, ValueType, ValueHandler, ReadWriteSizeType, Pattern> {
  ...
  public:
    ...
    void insert(const Pattern& pattern) override {
        data[pattern] = ValueType();
    } //singular insert required by PatternStore, implies 'default' ValueType, usually 0
    ...
    ValueType& operator[](const Pattern& pattern) override {
        return data[pattern];
    }
    ValueType& operator[](const PatternPointer& pattern) override {
        return data[pattern];
    }
...
class PatternPointerMap : public PatternMapStore<std::unordered_map<PatternPointer, ValueType>, ValueType, ValueHandler, ReadWriteSizeType, PatternPointer> {
  ...
  public:
    ...
    explicit PatternPointerMap() : corpus(NULL) {}
...
class OrderedPatternPointerMap : public PatternMapStore<std::map<PatternPointer, ValueType>, ValueType, ValueHandler, ReadWriteSizeType, PatternPointer> {
  ...
  public:
    ...
    explicit OrderedPatternPointerMap() : corpus(NULL) {}
...
class HashOrderedPatternMap : public PatternMapStore<std::map<const Pattern, ValueType>, ValueType, ValueHandler, ReadWriteSizeType, Pattern> {
  ...
  public:
    ...
    void insert(const Pattern& pattern) override {
        data[pattern] = ValueType();
    } //singular insert required by PatternStore, implies 'default' ValueType
...
class PatternStoreValueHandler : public AbstractValueHandler<PatternStoreType> {
  public:
    ...
    void convertto(PatternStoreType* source, PatternStoreType*& target) const {
        target = source;
    }; //noop
};
...
class AlignedPatternMap
  ...
  public:
    ...
    typedef
        typename PatternMap<PatternMap<ValueType, ValueHandler, NestedSizeType>, PatternStoreValueHandler<PatternMap<ValueType, ValueHandler, NestedSizeType>>, uint64_t>::iterator
                                                          iterator;
    typedef typename PatternMap<PatternMap<ValueType, ValueHandler, NestedSizeType>, PatternStoreValueHandler<PatternMap<ValueType, ValueHandler, NestedSizeType>>,
                                uint64_t>::const_iterator const_iterator;
};
