...
class AbstractAlignmentModel : public PatternMap<ValueType, ValueHandler>, public PatternModelInterface {
  ...
  public:
    ...
    virtual void load(std::istream& f, PatternModelOptions& options, PatternModelInterface* constrainmodel = NULL) { //load from file
        ...
        PatternMap<ValueType, ValueHandler>::template read(f, options.MINTOKENS, options.MINLENGTH, options.MAXLENGTH, constrainstore, !options.DOREMOVENGRAMS,
                                                           !options.DOREMOVESKIPGRAMS, !options.DOREMOVEFLEXGRAMS, options.DORESET, options.DEBUG);
        if (options.DEBUG)
            std::cerr << "Read " << this->size() << " patterns" << std::endl;
...
class PatternAlignmentModel : public AbstractAlignmentModel<PatternFeatureVectorMap<FeatureType>, PatternFeatureVectorMapHandler<FeatureType>> {
  public:
    ...
    PatternAlignmentModel(std::istream& f, PatternModelOptions& options, PatternModelInterface* constrainmodel = NULL) :
        AbstractAlignmentModel<PatternFeatureVectorMap<FeatureType>, PatternFeatureVectorMapHandler<FeatureType>>(f, options, constrainmodel) {}
    ...
    PatternAlignmentModel(const std::string& filename, const PatternModelOptions& options, PatternModelInterface* constrainmodel = NULL) :
        AbstractAlignmentModel<PatternFeatureVectorMap<FeatureType>, PatternFeatureVectorMapHandler<FeatureType>>(filename, options, constrainmodel) {}
    ...
    virtual PatternFeatureVector<FeatureType>* getfeaturevector(const Pattern& pattern, const Pattern& pattern2, bool makeifnew = false) {
        PatternFeatureVectorMap<FeatureType>* fvmap =
            AbstractAlignmentModel<PatternFeatureVectorMap<FeatureType>, PatternFeatureVectorMapHandler<FeatureType>>::getdata(pattern, makeifnew);
        if (fvmap == NULL)
            return NULL;
    ...
    void add(const Pattern& pattern, const Pattern& pattern2, std::vector<FeatureType>& features, bool checkifexists = true) {
        ...
        if (fv == NULL) {
            PatternFeatureVectorMap<FeatureType>* fvm =
                AbstractAlignmentModel<PatternFeatureVectorMap<FeatureType>, PatternFeatureVectorMapHandler<FeatureType>>::getdata(pattern, true);
...
class BasicPatternAlignmentModel : public AbstractAlignmentModel<PatternVector, PatternVectorHandler> {
  public:
    ...
    BasicPatternAlignmentModel(std::istream& f, PatternModelOptions& options, PatternModelInterface* constrainmodel = NULL) :
        AbstractAlignmentModel<PatternVector, PatternVectorHandler>(f, options, constrainmodel) {}

    BasicPatternAlignmentModel(const std::string& filename, const PatternModelOptions& options, PatternModelInterface* constrainmodel = NULL) :
        AbstractAlignmentModel<PatternVector, PatternVectorHandler>(filename, options, constrainmodel) {}
