...
class Pattern {
  public:
    ...
    unsigned char*
        data; /**< This array holds the variable-width byte representation, it is always terminated by \0 (ENDMARKER). Though public, you usually do not want to access it directly */
    ...
    Pattern(const PatternPointer& ref, size_t begin, size_t length, size_t* byteoffset = NULL, bool byteoffset_shiftone = false);
    ...
    bool            isskipgram() const {
        return category() == SKIPGRAM;
    }
    bool isflexgram() const {
        return category() == FLEXGRAM;
    }
    bool unknown() const;
...
class PatternPointer {
   ...
   */
    ...
    PatternPointer(const PatternPointer&, size_t, size_t, size_t* byteoffset = NULL, bool byteoffset_shiftone = false);
    PatternPointer(const Pattern&, size_t, size_t, size_t* byteoffset = NULL, bool byteoffset_shiftone = false);
    ...
    /**
      * Return a single token (not a byte!). index < size().
      */
    PatternPointer operator[](int index) const {
        return PatternPointer(*this, index, 1);
    }
